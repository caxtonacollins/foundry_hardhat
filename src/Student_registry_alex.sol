...
contract StudentRegister {
  ...
  function registerNewStudent(string memory _name) public studentDoesNotExist(msg.sender) {
    ...
    students[msg.sender] =
      Student({ Name: _name, attendance: Attendance.Absent, interest: new string[](0) });
  ...
  function removeInterest(address _addr, string memory _interest) public studentExists(_addr) {
    ...
    for (uint256 i = 0; i < students[_addr].interest.length; i++) {
    ...
    students[_addr].interest[indexToRemove] =
      students[_addr].interest[students[_addr].interest.length - 1];
