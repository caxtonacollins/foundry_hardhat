...
contract Payable {
    ...
    function getInvestment(
        address investor
    ) public view returns (uint256 investment) {
...
contract Funder {
    ...
    function sendWithSend(
        address payable _receiver
    ) public payable returns (bool sent) {
    ...
    function sendWithCall(
        address payable _receiver
    ) public payable returns (bool sent, bytes memory data) {
    ...
    function callDeposit(
        address payable _payableContractAddress
    ) public payable {
        uint256 amount = msg.value;
        (bool success, bytes memory data) = _payableContractAddress.call{
            value: amount
