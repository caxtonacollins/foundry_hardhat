...
contract PayableTest is Test {
    ...
    uint expected_initial_bal = 100 ether;
    uint expected_initial_addr1_bal = 0 ether;
    ...
    function test_deposit() public {
        uint initial_addr1_bal = address(addr1).balance;
        uint initial_payable_contract_bal = address(payable_contract).balance;
        uint deposit_amount = 1 ether;
        ...
        uint final_addr1_bal = address(addr1).balance;
        uint final_contract_bal = address(payable_contract).balance;
    ...
    function test_get_contract_balance() public view {
        ...
        assertEq(payable_contract.getContractBalance(), initialBalance, "Contract balance must be equal to zero");
    }
    ...
    function test_eth_with_transfer() public payable {
        address payable receiver = payable(address(payable_contract));
        uint initial_balance = address(payable_contract).balance;
        ...
        assertEq(final_addr1_bal, expected_initial_bal - _value, "Address1 balance should decrease by deposit amount");
    ...
    function test_send_with_call_deposit() public {
        ...
        assertEq(final_addr1_bal, expected_initial_bal - _value, "Address1 balance should decrease by deposit amount");
        assertEq(final_contract_bal, _value, "Contract balance should match deposit amount");
        assertEq(payable_contract.getInvestment(address(funder_contract)), _value, "Investment should be recorded");
    }
}